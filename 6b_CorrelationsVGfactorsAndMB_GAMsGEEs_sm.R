...
alldat = readMat('./OptimalSMS_rsfMRI/Results/CorrSDpower264_9p_sm.mat')
alldat_tf = readMat('./OptimalSMS_rsfMRI/Results/CorrSDpower264_9p_tf_sm.mat')
...
dim(alldat$allmb.sm.cor.srt)

nsubject = dim(alldat$allmb.sm.cor.srt)[4]
...
subj.cor.df = NULL
for (i in 1:nsubject) {
  temp = apply(alldat$allmb.sm.cor.srt[,,,i],3,net2vec)
  ...
  temp2 = apply(alldat$allmb.sm.gg.srt[,,,i],3,net2vec)
...
subj_tf.cor.df = NULL
for (i in 1:nsubject) {
  temp = apply(alldat_tf$allmb.tf.sm.cor.srt[,,,i],3,net2vec)
  temp.df = data.frame(atanh(temp))
  names(temp.df) = varnames_zcor
  temp.df$id = i
  temp.df$gender = subjectData2$Gender[i]
  ...
  temp2 = apply(alldat_tf$allmb.tf.sm.gg.srt[,,,i],3,net2vec)
  temp2.df = data.frame(temp2)
  names(temp2.df)=varnames_gg
  ptemp2=temp2.df%>%pivot_longer(cols=starts_with('gg_'),names_to='Acquisition2',names_prefix='gg_',values_to='gstar_factor')
  ptemp3 = cbind(ptemp,ptemp2[,2])
...
# itsdug::plot_smooth requires you to choose a level for each factor. Choose central node: 
median(tapply(subset.cor.df$zcor,subset.cor.df$PowerNode1Node2,FUN=mean,na.rm=TRUE))
# use 109_92


pdf(file='~/risk_share/OptimalSMS_rsfMRI/Documents/Figures/ZCor_Gfactor_gamm_sm.pdf')
plot_smooth(ggfactor_model,view='gstar_factor',rm.ranef=TRUE, rug=FALSE, xlab='g*-factor',ylab='Z Correlation',cex.lab=1.3,xlim=c(0.8,8),cond=list(PowerNode1Node2='109_92'),ylim=c(0,0.5))
dev.off()
...
for.table.geepack = function(out.model,region) {
  temp = summary(out.model)$coefficients[1:9,c(1,2,4)]
  temp2 = cbind(round(temp[,1:2],2),round(temp[,3],3))
  temp3 = c(paste0(temp2[1,1],'(',temp2[1,2],')'),paste0(temp2[2:9,1],'(',temp2[2:9,2],') ',ifelse(temp2[2:9,3]==0,'<0.001',temp2[2:9,3])))
  temp.data = data.frame(temp3)
  names(temp.data)=region
...
gee.results.9p = cbind(for.table.geepack(thalamus.model,'Thal 9p+sm'),for.table.geepack(putamen.model,'Puta 9p+sm'),for.table.geepack(cerebellum.model,'Cere 9p+sm'),for.table.geepack(executive.model,'Exec 9p+sm'),for.table.geepack(salience.model,'Sali 9p+sm'),for.table.geepack(dmn.model,'DMN 9p+sm'))

gee.results.9p.tf = cbind(for.table.geepack(thalamus_tf.model,'Thal 9p+bp+sm'),for.table.geepack(putamen_tf.model,'Puta 9p+bp+sm'),for.table.geepack(cerebellum_tf.model,'Cere 9p+bp+sm'),for.table.geepack(executive_tf.model,'Exec 9p+bp+sm'),for.table.geepack(salience_tf.model,'Sali 9p+bp+sm'),for.table.geepack(dmn_tf.model,'DMN 9p+bp+sm'))

print(xtable(gee.results.9p))
print(xtable(gee.results.9p.tf))
...
p0 = ggplot(thalamus, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Thalamus - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")


p0_tf = ggplot(thalamus_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Thalamus - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")


p1 = ggplot(putamen, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Putamen - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")

p1_tf = ggplot(putamen_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Putamen - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")

p2 = ggplot(cerebellum, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Cerebellum - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")
p2_tf = ggplot(cerebellum_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),aes(color=gstar_factor))+ggtitle('Cerebellum - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")

p3 = ggplot(executive, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('Executive - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")
p3_tf = ggplot(executive_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('Executive - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")

p4 = ggplot(salience, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('Salience - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")
p4_tf = ggplot(salience_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('Salience - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")

p5 = ggplot(dmn, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('DMN - 9p+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")
p5_tf = ggplot(dmn_tf, aes(x=Acquisition, y=zcor))+geom_boxplot(outlier.shape=NA) + geom_jitter(shape=16,position=position_jitter(0.2),size=0.75,aes(color=gstar_factor))+ggtitle('DMN - 9p+bandpass+sm')+ylab('Z Correlation')+scale_x_discrete(labels=c("SB.3.3.mm" = "SB3.3mm", "SB.2.mm" = "SB2mm","MB.2" = "MB2", "MB.3" = "MB3", "MB.4" = "MB4", "MB.6"="MB6","MB.8" = "MB8", "MB.9" = "MB9", "MB.12" = "MB12"))+scale_color_gradientn(colours=pal,limits=c(0.9,6),oob = scales::squish,name = "g*")



png(file='./OptimalSMS_rsfMRI/Documents/Figures/Boxplots_zcorrelations_9p_sm.png',width=13,height=12,units='in',res=500)
grid.arrange(p0,p1,p2,p3,p4,p5,nrow=3)
dev.off()

png(file='./OptimalSMS_rsfMRI/Documents/Figures/Boxplots_zcorrelations_9p_tf_sm.png',width=13,height=12,units='in',res=500)
